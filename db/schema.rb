...
ActiveRecord::Schema[7.2].define(version: 2026_02_01_193000) do
  ...
  create_table "appointment_requests", force: :cascade do |t|
    ...
    t.index ["guest_email"], name: "index_appointment_requests_on_guest_email_pending", unique: true, where: "(status = 0)"
    t.index ["nutritionist_id", "requested_at", "status"], name: "idx_requests_on_nutritionist_slot_status"
    t.index ["nutritionist_id", "requested_at"], name: "index_appointment_requests_on_nutritionist_slot_accepted", unique: true, where: "(status = 1)"
